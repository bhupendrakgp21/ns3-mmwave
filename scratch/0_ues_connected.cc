...
void MakeBaseStationSleep (Ptr<MmWaveSpectrumPhy> endlphy, Ptr<MmWaveSpectrumPhy> enulphy, bool val,uint32_t node_id)
{
 ...
 log_file_sleep_bs.open("log_file_0_ues_connected_sleep_time_bs.txt",std::ios::out | std::ios::app);
...
void EnergyConsumptionUpdate(uint32_t u_id, double totaloldEnergyConsumption, double totalnewEnergyConsumption)
{
 ...
 log_file_ue.open("log_file_0_ues_connected_handover_energy_consumption.txt",std::ios::out | std::ios::app);
...
void EnergyConsumptionUpdateBS(uint32_t b_id, double totaloldEnergyConsumption, double totalnewEnergyConsumption)
{
 ...
 log_file_bs.open("log_file_bs_0_ues_connected_handover_energy_consumption.txt",std::ios::out | std::ios::app);
...
uint32_t num_of_appUEs = 10;
 ...
 enbPositionAlloc->Add (Vector (100,100,10)); // LTE BS, out of area where buildings are deployed
...
MobilityHelper appUEMobility, noAppUEMobility;
  Ptr<ListPositionAllocator> appUEPositionAlloc = CreateObject<ListPositionAllocator> ();
  Ptr<ListPositionAllocator> noAppUEPositionAlloc = CreateObject<ListPositionAllocator> ();
  ...
  for(uint32_t i = 0; i < appUENodes.GetN(); i ++)
    {
      srand(time(0));
      x_random = ((rand()+int(y_random)) % 983) + 10;
      y_random = ((rand()+int(x_random)) % 983) + 10;
      std::cout<<"UEs position :"<<i<<' '<<x_random<<' '<<y_random<<std::endl;
      appUEPositionAlloc->Add (Vector (x_random, y_random, 1.5));
    }

    for(uint32_t i = 0; i < noAppUENodes.GetN(); i ++)
    {
      srand(time(0));
      x_random = ((rand()+int(y_random)) % 983) + 10;
      y_random = ((rand()+int(x_random)) % 983) + 10;
      std::cout<<"UEs position :"<<i<<' '<<x_random<<' '<<y_random<<std::endl;
      noAppUEPositionAlloc->Add (Vector (x_random, y_random, 1.5));
    }

    

  //uemobility.SetMobilityModel ("ns3::ConstantVelocityMobilityModel");
   appUEMobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
       "Bounds", RectangleValue (Rectangle (0, 1000, 0, 1000)),
       "Speed", StringValue("ns3::UniformRandomVariable[Min=" + std::to_string(5) + "|Max=" + std::to_string(30) + "]"));
  appUEMobility.SetPositionAllocator (appUEPositionAlloc);
  appUEMobility.Install (appUENodes);

  noAppUEMobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
       "Bounds", RectangleValue (Rectangle (0, 1000, 0, 1000)),
       "Speed", StringValue("ns3::UniformRandomVariable[Min=" + std::to_string(5) + "|Max=" + std::to_string(30) + "]"));
  noAppUEMobility.SetPositionAllocator (noAppUEPositionAlloc);
  noAppUEMobility.Install (noAppUENodes);
 ...
 NetDeviceContainer appMcUeDevs;
  appMcUeDevs = mmwaveHelper->InstallMcUeDevice (appUENodes);

  // Install the IP stack on the UEs
  internet.Install (appUENodes);
  Ipv4InterfaceContainer appUEIpIface;
  appUEIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (appMcUeDevs));
  // Assign IP address to UEs, and install applications
  for (uint32_t u = 0; u < appUENodes.GetN (); ++u)
    {
      Ptr<Node> ueNode = appUENodes.Get (u);
      // Set the default gateway for the UE
      Ptr<Ipv4StaticRouting> ueStaticRouting = ipv4RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv4> ());
      ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
    }

  for(uint32_t i = 0; i<appMcUeDevs.GetN();i++)
    {
        uint64_t imsi = appMcUeDevs.Get(i)->GetObject<McUeNetDevice>()->GetImsi();
        Ue_Imsi_to_nodeID[imsi]= i;
    }

     NetDeviceContainer noAppMcUeDevs;
  noAppMcUeDevs = mmwaveHelper->InstallMcUeDevice (noAppUENodes);

  internet.Install (noAppUENodes);
  Ipv4InterfaceContainer noAppUEIpIface;
  noAppUEIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (noAppMcUeDevs));
  // Assign IP address to UEs, and install applications
  for (uint32_t u = 0; u < noAppUENodes.GetN (); ++u)
    {
      Ptr<Node> ueNode = noAppUENodes.Get (u);
      // Set the default gateway for the UE
      Ptr<Ipv4StaticRouting> ueStaticRouting = ipv4RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv4> ());
      ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
    }

  for(uint32_t i = 0; i<noAppMcUeDevs.GetN();i++)
    {
        uint64_t imsi = noAppMcUeDevs.Get(i)->GetObject<McUeNetDevice>()->GetImsi();
        Ue_Imsi_to_nodeID[imsi]= i;
    }
  
  for(uint32_t i = 0; i<mmWaveEnbDevs.GetN();i++)
    {
        uint64_t cellid = mmWaveEnbDevs.Get(i)->GetObject<MmWaveEnbNetDevice>()->GetCellId();
        BaseSt_CellId_to_nodeId[cellid]= i;
    }
 // Add X2 interfaces
 mmwaveHelper->AddX2Interface (lteEnbNodes, mmWaveEnbNodes);

 // Manual attachment
  mmwaveHelper->AttachToClosestEnb (appMcUeDevs, mmWaveEnbDevs, lteEnbDevs);
  mmwaveHelper->AttachToClosestEnb (noAppMcUeDevs, mmWaveEnbDevs, lteEnbDevs);
 ...
 for(uint32_t i = 0 ; i < appUENodes.GetN() ; i++)
 {
 Ptr<McUeNetDevice> mcuedev = appUENodes.Get(i)->GetDevice (0)->GetObject <McUeNetDevice> ();
 ...
 for(uint32_t i = 0 ; i < noAppUENodes.GetN() ; i++)
 {
 Ptr<McUeNetDevice> mcuedev = noAppUENodes.Get(i)->GetDevice (0)->GetObject <McUeNetDevice> ();
 std::cout<<"IMSI of UE node id = "<< i << " IMSI = "<< mcuedev->GetImsi();
 Ue_Imsi_to_nodeID[mcuedev->GetImsi()] = i;
 connected_mmWave[i] = -1;
 std::cout<<std::endl;
...
//**********************************************************************************************************************************
 ...
 // Install Energy Source
  EnergySourceContainer appSources = basicSourceHelper.Install (appUENodes);
  EnergySourceContainer noAppSources = basicSourceHelper.Install (noAppUENodes);
 ...
 DeviceEnergyModelContainer appDeviceEnergyModel = nrEnergyHelper.Install (appMcUeDevs, appSources);
  DeviceEnergyModelContainer noAppDeviceEnergyModel = nrEnergyHelper.Install (noAppMcUeDevs, noAppSources);
 ...
 // Install Energy Source
 //check here
 appDeviceEnergyModel.Get(u)->TraceConnectWithoutContext ("TotalEnergyConsumption", MakeBoundCallback (&EnergyConsumptionUpdate, u));
 }
 ...
 // Install Energy Source
 //check here
 noAppDeviceEnergyModel.Get(u)->TraceConnectWithoutContext ("TotalEnergyConsumption", MakeBoundCallback (&EnergyConsumptionUpdate, u));
 }
      ...
      if (dl)
        {
          ...
          serverApps.Add (dlPacketSinkHelper.Install (appUENodes.Get (u)));
          ...
          UdpClientHelper dlClient (appUEIpIface.GetAddress (u), dlPort);
      ...
      if (ul)
        {
          ...
          clientApps.Add (ulClient.Install (appUENodes.Get (u)));
        }
    }
