...
void EnergyConsumptionUpdate(uint32_t u_id, double totaloldEnergyConsumption, double totalnewEnergyConsumption)
{
 ...
 log_file_ue.open("log_file_application_aware_handover_energy_consumption.txt",std::ios::out | std::ios::app);
...
void EnergyConsumptionUpdateBS(uint32_t b_id, double totaloldEnergyConsumption, double totalnewEnergyConsumption)
{
 ...
 log_file_bs.open("log_file_bs_application_aware_handover_energy_consumption.txt",std::ios::out | std::ios::app);
...
//  deployEnb(p,num_of_mmWaveEnb);
 ...
 for(int i = 0; i < num_of_mmWaveEnb; i++)
 {
    ...
    std::cout<<"mmwave bs position for "<<i<<"th node: "<<p[i][0]<<" "<<p[i][1]<<std::endl;
 }
...
MobilityHelper appUEMobility, noAppUEMobility;
  ...
  noAppUEMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
 ...
 std::cout<<"IMSI of app UE node id = "<< i << " IMSI = "<< mcuedev->GetImsi();
 ...
 std::cout<<"IMSI of no app UE node id = "<< i << " IMSI = "<< mcuedev->GetImsi();
 Ue_Imsi_to_nodeID[mcuedev->GetImsi()] = i;
 connected_mmWave[i] = -1;
 std::cout<<std::endl;
 }
...
//**********************************************************************************************************************************
 ...
 MakeCallback (&NotifyHandoverEndOkUe));
  ...
  MakeBaseStationSleep(enbdl, enbul, true, 0);
